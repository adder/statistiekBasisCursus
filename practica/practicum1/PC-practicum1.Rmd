...
We zullen nu de dataset lovett.dat inlezen. Dat kan op verschillende manieren:

 - via de link naar de dataset op onze website (https://statomics.github.io/statistiekBasisCursus/), dit is erg makkelijk aangezien je de dataset niet manueel hoeft te downloaden en diens locatie niet hoeft aan te duiden in R.
 - Manueel downloaden en inlezen
    + via Import dataset (rechts bovenaan in RStudio) > From Text File
    + via de `read.table` functie zoals onderaan gebeurt. Echter, als je de dataset manueel hebt gedownload, dan moet je aan R verduidelijken waar je de dataset hebt opgeslaan. Indien je de dataset bijvoorbeeld hebt op opgeslaan in de folder R op je H-schijf, dan kan je de path variabele aanpassen als `path <- "H:/R/lovett.dat"`. Dit commando neemt aan dat de `lovett.dat` file zich in `H:/R/` bevindt; dus op je H schijf in folder R. Indien dit niet zo is, verander je dit pad naar de correcte locatie.
...
```{r}
NH4 <- lovett$NH4
NH4CAT <- ifelse(NH4 < 1.5, 0, 1) # 0: NH4 < 1.5; 1: NH4 >= 1.5
table(NH4CAT)
